...
while ($row = mysqli_fetch_assoc($GETLIST)) {
    ...
    if (($fillnumb  == 3 ) ||  ($fillnumb  == 0 ) ||  ($fillnumb  == 1 )) {
       echo " <Tr><td>$illNUB</td><Td>$title</td><td>$author</td><td>$itype</td><td>$reqnote</td><td>$lendnote</td><td>$needby</td><td>$dest</td><td>$reqp<br>$reql</td><td>$timestamp</td><td>$fill</td>
     <td>
       <a href='https://duenorth.nnyln.org/respond?num=$illNUB&a=1'>Yes</a>
        <br><br>
        <a href='https://duenorth.nnyln.org/respond?num=$illNUB&a=0'>No</a>
       </td></tr> ";
      }else{
       echo " <Tr><td>$illNUB</td><Td>$title</td><td>$author</td><td>$itype</td><td>$reqnote</td><td>$lendnote</td><td>$needby</td><td>$dest</td><td>$reqp<br>$reql</td><td>$timestamp</td><td>$fill</td><td>&nbsp</td></tr> ";
     }
   }
echo "</table>";
 if(( $page > 0 ) && (($offset +  $rec_limit)<$GETLISTCOUNTwhole))
         {
            $last = $page - 2;
            echo "<a href='senthistory?page=$last&loc=".$loc." '>Last 10 Records</a> |";
            echo "<a href='senthistory?page=$page&loc=".$loc." '>Next 10 Records</a>";
         }
