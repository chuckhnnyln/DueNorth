...
while ($row = mysqli_fetch_assoc($GETLIST)) {
  ...
  if ($rowtype & 1 ) {
    echo "<tr class='odd'><td>$illNUB</td><Td>$title</td><td>$author</td><td>$itype</td><td>$avail</td><td>$reqnote</td><td>$respnote<td>$needby</td><td>$dest</td><td>$reqp<br>$reql</td><td>$timestamp</td><td>$fill</td></tr> ";
  } else {
    echo "<tr class='even'><td>$illNUB</td><Td>$title</td><td>$author</td><td>$itype</td><td>$avail</td><td>$reqnote</td><td>$respnote<td>$needby</td><td>$dest</td><td>$reqp<br>$reql</td><td>$timestamp</td><td>$fill</td></tr> ";
  }
  $rowtype = $rowtype + 1;
}
echo "</table>";
if(( $page > 0 ) && (($offset +  $rec_limit)<$GETLISTCOUNTwhole)) {
  $last = $page - 2;
  echo "<a href='allrequests?page=$last\'>Last 25 Records</a> |";
  echo "<a href='allrequests?page=$page\'>Next 25 Records</a>";
