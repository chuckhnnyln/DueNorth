...
#Filter options
echo "<form action='borrower-history' method='post'>";
...
echo "</select> ";
echo "<a href='borrower-history?loc=$loc'>clear</a>  ";
...
echo "<table><TR><TH width='5%'>ILL #</TH><TH width='25%'>Title / Author</TH><TH>Type</TH><TH>Need By</TH><TH>Lender Destination & Contact</TH><TH>Borrower</TH><TH>Timestamp</TH><TH>Fill</TH><TH>Action</TH></TR>";
$rowtype=1;
while ($row = mysqli_fetch_assoc($GETLIST)) {
  ...
  if ($fillnumb == 3 ){
    ...
    echo "<TR class='$rowclass'><TD>$illNUB</TD><TD>$title</br><i>$author</i></TD><TD>$itype</TD><TD>$needby</TD><TD><a href='mailto:$destemail?Subject=NOTE Request ILL# $illNUB' target='_blank'>$dest</a></TD><TD>$reqp</TD><TD>$timestamp</TD><TD>$fill</TD><TD><a href ='https://duenorth.nnyln.org/cancel?num=$illNUB&a=3'>Cancel Request</a></TD></TR> ";
    if ( (strlen ($reqnote) > 2) || (strlen ($lendnote) > 2) ) echo "<TR class='$rowclass'><TD></TD><TD></TD><TD colspan=7>$displaynotes</TD></TR>";

  } else {
    echo "<TR class='$rowclass'><TD>$illNUB</TD><TD>$title</br><i>$author</i></TD><TD>$itype</TD><TD>$needby</TD><TD><a href='mailto:$destemail?Subject=NOTE Request ILL# $illNUB' target='_blank'>$dest</a></TD><TD>$reqp</TD><TD>$timestamp</TD><TD>$fill</TD><TD>&nbsp</TD></TR> ";
    if ( (strlen ($reqnote) > 2) || (strlen ($lendnote) > 2) ) echo "<TR class='$rowclass'><TD></TD><TD></TD><TD colspan=7>$displaynotes</TD></TR>";
  }
  $rowtype = $rowtype + 1;
}
