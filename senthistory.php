   ...
   if (strlen($dest)>2){
    ...
    if ($fillnumb == 3 ){
      #Only show cancel button if request has not been answered
       echo " <Tr><td>$illNUB</td><Td>$title</td><td>$author</td><td>$itype</td><td>$reqnote</td><td>$lendnote</td><td>$needby</td><td>$dest</td><td>$destemail</td><td>$reqp<br>$reql</td><td>$timestamp</td><td>$fill</td>
      <td><a href ='https://duenorth.nnyln.org/cancel?num=$illNUB&a=3'>Cancel Request</a></td></tr> ";
      }else{
         echo " <Tr><td>$illNUB</td><Td>$title</td><td>$author</td><td>$itype</td><td>$reqnote</td><td>$lendnote</td><td>$needby</td><td>$dest</td><td>$destemail</td><td>$reqp<br>$reql</td><td>$timestamp</td><td>$fill</td><td>&nbsp</td></tr> ";
    }
   }
echo "</table>";
 ...
 if(( $page > 0 ) && (($offset +  $rec_limit)<$GETLISTCOUNTwhole))
         {
            $last = $page - 2;
            echo "<a href='requesthistory?page=$last&loc=".$loc." '>Last 10 Records</a> |";
            echo "<a href='requesthistory?page=$page&loc=".$loc." '>Next 10 Records</a>";
         }
